  ...
  const requests = {
    generic: {
      query(query) {
        ...
        return fetch(`${store.state.constants.SERVER_URL}`, {
          method: 'POST',
          mode: 'cors',
          credentials: 'include',
          headers: {
            ...
            'Access-Control-Allow-Origin': store.state.constants.CLIENT_URL,
    ...
    get: {
      async Recipe({ store, params }) {
        if (params.id && store.state.query) {
          ...
          return requests.generic.query(query)
            .then((response) => response.data && response.data.recipe);
        }
        return undefined;
      },
      async Recipes(searchQuery, originalFrom) {
        ...
        return requests.generic.query(query)
          .then((response) => response.data && response.data.recipes);
      },
      async Favorites() {
        const query = 'query {getFavorites{id title thumbnail previewText}}';
        return requests.generic.query(query)
          .then((response) => response.data && response.data.getFavorites);
      },
    },
    add: {
      async Favorite(id) {
        ...
        return requests.generic.query(query)
          .then((response) => response.data && response.data.addFavorite);
      },
    },
    remove: {
      async Favorite(id) {
        ...
        return requests.generic.query(query)
          .then((response) => response.data && response.data.removeFavorite);
      },
    },
  };
